  ...
  const compareMimeType = (mimeTypes, fileType, formatFile) => {
      ...
      for (let index = 1; index < mimeTypes.length; index += 3) {
          if ((mimeTypes[index + 1] === "*" && fileType.search(new RegExp(mimeTypes[index])) !== -1) ||
              (mimeTypes[index + 1] && mimeTypes[index + 1] !== "*" && (fileType.search(new RegExp("\\*" + mimeTypes[index + 1] + "\\*")) !== -1 || fileType.search(new RegExp(mimeTypes[index + 1])) !== -1)) ||
              (mimeTypes[index + 1] === "" && (fileType.search(new RegExp(mimeTypes[index])) !== -1 || formatFile.search(new RegExp(mimeTypes[index])) !== -1))) {
              return true;
          }
      }
